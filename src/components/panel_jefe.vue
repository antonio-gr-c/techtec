<template>
  <div class="row vh-100 no-gutters">
    ...
    <div class="col-12 col-md-9 col-lg-10 main-content">
      <div class="content mt-4">
        <div class="d-flex justify-content-start mb-4">
          ...
          />
          <button class="btn btn-secondary ms-3" @click="goToTrabajadores">
              ...
              <tr v-for="equipo in orderedEquipos" :key="equipo.id">
                ...
                <td>
                  <button
                    ...
                    @click="openWorkerModal(equipo)"
                    data-bs-toggle="modal"
                    :data-bs-target="'#modalTrabajadores' + equipo.id"
                  >
                    {{
                      equipo.trabajadoresAsignados?.length > 0
                        ? equipo.trabajadoresAsignados.map((t) => t.nombre).join(", ")
                        : "Sin asignar"
                    }}
                  </button>
                </td>
                  ...
                  <div class="etiquetas">
                    <span v-if="equipo.etiquetas?.length > 0">
                      <span v-for="(etiqueta, index) in equipo.etiquetas" :key="index">
                        {{ etiqueta
                        }}<span v-if="index < equipo.etiquetas.length - 1">, </span>
                      </span>
                    </span>
                ...
                <td>
                  <div class="info-button-panel">
                    <button
                      class="btn btn-primary ms-2 info-button"
                    ...
                    <button
                      class="btn btn-danger ms-2 info-button"
                      @click="deleteService(equipo.id)"
                    >
                      <span class="material-symbols-outlined">delete</span>
                    </button>
                  </div>
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-dark text-white">
          <div class="modal-header">
            <h5 class="modal-title" :id="'modalLabel' + equipo.id">
              {{ equipo.nombre }}
            </h5>
            <button
              type="button"
            ...
            <table class="table table-dark table-bordered text-white">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a
                      :href="`mailto:${equipo.cliente?.email}`"
                      class="text-info text-decoration-none"
                    >
    ...
    <div
      v-for="equipo in equipos"
      :key="'modalTrabajadores' + equipo.id"
      class="modal fade"
      :id="'modalTrabajadores' + equipo.id"
      tabindex="-1"
      aria-labelledby="'modalLabelTrabajadores' + equipo.id"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-dark text-white">
          <div class="modal-header">
            <h5 class="modal-title" :id="'modalLabelTrabajadores' + equipo.id">
              Trabajadores
            </h5>
            <button
              type="button"
            ...
            <div class="row">
              <div class="col-6">
                ...
                <table class="table table-bordered table-dark text-white">
                  ...
                  <tbody>
                    <tr
                      v-for="trabajador in equipo.trabajadoresAsignados || []"
                      :key="trabajador.id"
                    >
                      <td>{{ trabajador.id }}</td>
                      ...
                      <td>
                        <button
                          ...
                          @click="desasignarTrabajador(equipo, trabajador)"
                        >
                          Desasignar
                        </button>
                      </td>
              ...
              <div class="col-6">
                ...
                <table class="table table-bordered table-dark text-white">
                  ...
                  <tbody>
                    <tr
                      v-for="trabajador in trabajadoresDisponibles(equipo)"
                      :key="trabajador.id"
                    >
                      <td
                        :class="{
                      ...
                      <td>
                        <button
                          ...
                          :disabled="trabajador.estado !== 'disponible'"
                          :class="{ 'btn-secondary': trabajador.estado !== 'disponible' }"
                          @click="asignarTrabajador(equipo, trabajador)"
                        >
                          Asignar
                        </button>
                      </td>
...
export default {
  ...
  setup() {
    ...
    const fetchEquipos = async () => {
      try {
        ...
        equipos.value = response.data.map((equipo) => ({
          ...
          cliente: equipo.cliente || {}, // Asegúrate de que contenga los datos del cliente
    ...
    const fetchTrabajadores = async () => {
      try {
        ...
        trabajadores.value = response.data.map((trabajador) => ({
          ...
          equiposAsignados: trabajador.equiposAsignados,
    ...
    // Función para obtener las asignaciones de trabajadores a servicios
    const fetchAsignaciones = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/servicio/asignaciones"
        );
  ...
  try {
    // Asignar al trabajador en el equipo
    ...
    await axios.post("http://localhost:3000/notificaciones", {
      ...
      id_servicio: equipo.id,
      id_trabajador: trabajador.id,
      id_destinatario: trabajador.id,
    });

    equipo.trabajadoresAsignados.push(trabajador);
    Swal.fire("Trabajador asignado", "Notificación enviada correctamente.", "success");
  } catch (error) {
    ...
    // Función para desasignar un trabajador
  ...
  try {
    // Desasignar al trabajador del equipo
    await axios.post("http://localhost:3000/api/desasignar-trabajador", {
      id_servicio: equipo.id,
      id_trabajador: trabajador.id,
    });

    // Enviar notificación de desasignación
    await axios.post("http://localhost:3000/notificaciones", {
      tipo: "desasignacion",
      id_servicio: equipo.id,
      id_trabajador: trabajador.id,
    ...
    equipo.trabajadoresAsignados = equipo.trabajadoresAsignados.filter(
      (t) => t.id !== trabajador.id
    );
    Swal.fire("Trabajador desasignado", "Notificación enviada correctamente.", "success");
  } catch (error) {
    console.error("Error al desasignar trabajador:", error);
    Swal.fire(
      "Error al desasignar trabajador",
      error.response?.data?.error || "",
      "error"
    );
  }
...
.btn-secondary {
  background-color: #6c757d; /* Color gris clásico */
  border: none;
  color: white;
  font-weight: bold;
  padding: 10px 20px;
...
.btn-secondary:active {
  background-color: #4e555b; /* Color más oscuro al presionar */
  transform: translateY(1px); /* Simula el botón presionado */
}

.btn-secondary:focus {
  outline: none;
  box-shadow: 0 0 5px rgba(0, 0, 0, 0.3); /* Agrega un sutil efecto de enfoque */
}
</style>
